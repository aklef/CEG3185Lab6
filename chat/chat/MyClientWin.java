...
public class MyClientWin extends Applet implements Runnable
{
		...
		else if (arg == "Connect" && !bConnected)
		{
			try
			{
				...
				String received = serverConnection.read();
                                NetworkFrame snrm = new NetworkFrame(received);
                                
                                if (snrm.getFrameType() != Type.UFrame || snrm.getControlCode() != ControlCode.SNRM)
                                {
                                    System.out.println("ERROR : Did not receive SNRM frame from " + serverConnection.getAddress());
                                }
                                
                                NetworkFrame ua = new NetworkFrame((Object)serverConnection.getAddress(), Type.UFrame, ControlCode.UA);
	...
	public void checkServer()
	{
		...
		try
		{
			...
			{
				NetworkFrame receivedFrame = new NetworkFrame(fromServer);
				...
				switch(receivedFrame.getFrameType())
				{
					...
					case SFrame:
						if (receivedFrame.getControlCode() == ControlCode.RR)
                                                {
                                                    //Something to send
                                                    if (fromUser != null)
                                                    {
                                                        NetworkFrame toSend = new NetworkFrame((Object)serverConnection.getAddress(), Type.IFrame, ControlCode.RR, fromUser);
                                                    ...
                                                    {
                                                       NetworkFrame toSend = new NetworkFrame((Object)serverConnection.getAddress(), Type.SFrame, ControlCode.RR); 
                                                       serverConnection.send(toSend.toString()); 
                                                    }
                                                }
                                                else
